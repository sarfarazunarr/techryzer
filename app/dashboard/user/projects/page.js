...
const page = async () => {
    ...
    const id = session.id;
    const datasession = await fetchApiData(`/api/projects/${id}`);
    const data = datasession.projects;
    if (data.length == 0) {
        return (
            <div className='flex flex-col justify-center items-center'>
                <h3 className='text-white font-semibold text-3xl py-3 pb-4'>No Project Found</h3>
                <Link href='/dashboard/user/projects/new' className='text-white font-semibold bg-primary-600 text-sm rounded-md px-5 py-3 hover:bg-primary-800'>Create New Project</Link>
            </div>
        )
    }
    return (
