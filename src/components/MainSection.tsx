...
const MainSection = ({title, content, image, alt, link, btnText, span}: {title: string, content: string, image: string, alt: string, link:string, btnText: string, span?:string}) => {
    return (
        <section className='flex items-center mx-32'>
            <div className='flex flex-col justify-start items-start gap-y-4'>
                ...
                <h2 className='text-4xl font-semibold text-white font-outfit text-left'>{title}</h2>
                <p className='text-xl text-white font-manrop leading-relaxed'>{content}</p>
